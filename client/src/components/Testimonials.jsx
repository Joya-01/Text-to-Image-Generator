import React from 'react'
import {  motion } from "motion/react"

import {assets, testimonialsData} from "../assets/assets/assets"

const Testimonials = () => {
  return (
    <motion.div 
    initial={{ opacity: 0.2, y: 100 }}
    transition={{ duration: 1 }}
    whileInView={{ opacity: 1, y: 0 }}
    viewport={{ once: true }}
    
    className='flex flex-col items-center justify-center my-24 p-12'>
        
        <h1 className='text-3xl sm:text-4xl font-semibold mb-2'>
            Customer Testimonials
        </h1>
        <p className=' text-gray-600 mb-12'>
            What Our Users Are Saying
        </p>

        <div className='flex flex-wrap gap-6'>
            {testimonialsData.map((items,idx)=>(
                <div key={idx} className='bg-white/20 p-12 rounded-lg shadow-md border:w-80 m-auto cursor-pointer hover:shadow-lg transition-lg duration-300 w-80'>
                    <div className='flex flex-col items-center '>
                        <img src={items.image} alt="" className='rounded-full w-14'/>
                        <h2 className='text-xl font-semibold mt-3'>{items.name}</h2>
                        <p className='mb-4 text-gray-500'>{items.role}</p>
                        <div className='flex mb-4'>
                            {Array(items.stars).fill().map((item,index)=>(
                                <img key={index} src={assets.rating_star} alt="" />
                            ))}
                        </div>
                        <p className='text-center text-sm text-gray-600'>{items.text}</p>
                    </div>
                </div>
            ))}
        </div>
    </motion.div>
  )
}

export default Testimonials